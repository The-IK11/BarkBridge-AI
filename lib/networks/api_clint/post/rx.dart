...
/// A reactive class for handling POST API requests with built-in error handling,
/// success callbacks, and toast notifications.
///
/// This class extends [RxResponseInt] and provides a streamlined way to make
/// POST requests to REST APIs. It automatically handles common scenarios like
...
/// final postUser = PostRx<UserModel>(
...
/// - Success callback support for post-processing
...
final class PostRx<T> extends RxResponseInt<T> {
  final api = PostApi.instance;

  /// Creates a new PostRx instance for handling POST API requests.
  ///
  /// ## Parameters
  ///
  ...
  /// - [endPoint]: The API endpoint path for the POST request (e.g., '/users/login').
  ///   Can be overridden when calling [postData].
  ...
  PostRx({
    ...
    this.onSuccess,
    this.onError,
    this.toastSetting = ToastSetting.error,
    this.feedbackType = FeedbackType.toast,
    this.errorKey,
  ...
  /// The API endpoint path for POST requests.
  ///
  /// This is the default endpoint used when calling [postData] without
  ...
  /// Controls the type of feedback UI shown (toast or dialog).
  ...
  /// and can be used for post-processing like storing tokens, updating UI state, or navigation.
  ...
  ValueStream get postResponseStream => dataFetcher.stream;

  /// Makes a POST request to the specified API endpoint.
  ///
  /// This is the primary method for executing POST API calls. It handles
  ...
  ///
  /// ```dart
  /// bool success = await postLogin.postData(
  ///   data: {
  ...
  Future<bool> postData({
    ...
    try {
      final Map resdata = await api.post(
        data: data,
  ...
  ///
  /// This method is called internally when a POST request succeeds. It processes
  ...
  handleSuccessWithReturn(data) async {
    ...
    if (toastSetting == ToastSetting.both ||
        toastSetting == ToastSetting.successfull) {
      if (feedbackType == FeedbackType.dialog) {
  ...
  ///
  /// This method is called internally when a POST request fails. It processes
    ...
    if (error is DioException) {
      ...
      if (error.type == DioExceptionType.connectionError) {
        ...
        if (responseData is Map) {
          final dynamic messageFromResponse =
              responseData[errorKey ?? 'message'] ?? responseData['message'];
          if (messageFromResponse != null) {
            apiMessage = messageFromResponse.toString().trim();
          }

          final dynamic validationSource = responseData['data'];
          if (validationSource is Map || validationSource is String) {
            validationMessage = convertValidationErrorsToReadable(
              validationSource,
            ).trim();
          }
